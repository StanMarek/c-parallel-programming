...
main ( int argc, char** argv )
{
  ...
  for(i=0;i<WYMIAR;i++) x[i]=0.0;
                                                                                                            ...
                                                                                                            //inicjowanie danych wejsciowych
  if(rank==0){  
  ...
  if(size>1){
	...
	MPI_Recv( a_local, n_wier*WYMIAR, MPI_DOUBLE, source, MPI_ANY_TAG, MPI_COMM_WORLD, &status );
	MPI_Recv( &x[rank*n_wier], n_wier, MPI_DOUBLE, source, MPI_ANY_TAG, MPI_COMM_WORLD, &status );
	
      } else {
	
        MPI_Recv( a_local, n_wier_last*WYMIAR, MPI_DOUBLE, source, MPI_ANY_TAG, MPI_COMM_WORLD, &status );
        MPI_Recv( &x[(size-1)*n_wier], n_wier_last, MPI_DOUBLE, source, MPI_ANY_TAG, MPI_COMM_WORLD, &status );	
    ...
    MPI_Scatter(&a[rank*WYMIAR*n_wier], n_wier*WYMIAR, MPI_DOUBLE, a_local, n_wier*WYMIAR, MPI_DOUBLE, 0, MPI_COMM_WORLD );
    
    double* b = (double *) malloc((WYMIAR)*sizeof(double));
    for(int j=0; j<WYMIAR; j++)
        b[j] = x[j];
    
    MPI_Scatter(b, n_wier, MPI_DOUBLE, &x[rank*n_wier], n_wier, MPI_DOUBLE, 0, MPI_COMM_WORLD );
    ...
    //MPI_Barrier(MPI_COMM_WORLD);  
    double* c = (double *) malloc((WYMIAR)*sizeof(double));
    for(int j=0; j<WYMIAR; j++)
        c[j] = z[j];
    MPI_Gather(c, n_wier, MPI_DOUBLE, &z[rank*n_wier], n_wier, MPI_DOUBLE, 0, MPI_COMM_WORLD);
   // MPI_Barrier(MPI_COMM_WORLD);
